...
class QueueTransferTest {
    ...
    @Test fun largeListTransfersWholeOrderAndDuplicatesThroughBoundedPagesAndRestoresExactSelection() {
        ...
        val candidate = store.read(reference, owner, 3001)
        ...
        assertThrows(IllegalArgumentException::class.java) { store.read(reference, owner, 0) }
    }

    @Test fun incompleteOrChangedRevisionCannotBePublishedAndSelectionDoesNotInvalidateMetadataPages() {
        val queue = PlaybackQueue().apply { replace(List(100) { song(it) }, 0) }
    ...
    @Test fun invalidTransfersAndLateReplacementLeaveTheActiveQueueUntouched() {
        ...
        val ticket = requests.begin(owner, 7)
        val songs = listOf(song(1), song(2), song(1))
        store.write(ticket.reference, owner, songs)
        assertThrows(IllegalArgumentException::class.java) { store.read(ticket.reference, "other-owner", 0) }
        assertThrows(IllegalArgumentException::class.java) { store.read(ticket.reference, owner, 3) }
        assertThrows(IllegalArgumentException::class.java) { store.read("../outside", owner, 0) }
        // An IO result which arrives after a newer queue intent cannot commit.
        val late = store.read(ticket.reference, owner, 2)
        requests.invalidate()
        if (requests.accepts(ticket, owner, 7)) queue.restore(late)
        assertEquals(before, queue.snapshot())
        val newer = requests.begin(owner, 7)
        ...
        assertThrows(RuntimeException::class.java) { store.read(newer.reference, owner, 0) }
        assertEquals(before, queue.snapshot())
        store.clear()
    ...
    @Test fun encodedSizeLimitsRejectUnpageableItemsAndSnapshotsTooLargeToRestore() {
        ...
        assertThrows(IllegalArgumentException::class.java) { store.read(reference, owner, 0) }
        val queue = PlaybackQueue().apply { replace(songs, 0) }
        assertThrows(IllegalArgumentException::class.java) {
