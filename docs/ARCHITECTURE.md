...
完整列表替换由控制器先取得服务签发的请求引用，IO 将歌曲元数据写入按会话隔离的私有临时文件，再通过 MediaController 提交引用和选中索引。QueueTransferSession 在 IO 读取并校验完整内容与快照尺寸，回到服务线程复核请求及账号版本后一次提交。新队列操作会取消旧替换；失败保留旧播放，提交后的音频失败则保留新队列。临时记录在完成、取消、账号变更及启动时清理。

会话 extras 只包含当前项、数量、播放状态及队列内容版本；QueueController 仅在队列面板可见时通过服务命令读取 QueuePages 的有界 JSON 分页，版本变化丢弃旧批次，完整读取后才更新列表。进度轮询不重新解码整表。临时文件和恢复快照均按实际编码字节检查 4 MiB 上限，分页为 24 KiB；超过限制明确失败，不静默截断。普通/FM 基础状态边界已实现，详见 QUEUE_PREPARATION_PLAN.md。

## 队列会话类型边界

QueueSessionType（NORMAL / FM）与 PlaybackMode（顺序 / 随机 / 单曲循环）独立。插入和点选不改会话类型；完整列表替换的调用者必须明确目标类型，服务将目标绑定到替换请求，在完整校验后与队列原子提交。清理活动队列重置为 NORMAL，不保留旧队列备份。会话 extras 和 PlaybackUiState 提供当前类型，快照保存该字段；旧 v1/v2 缺失字段时兼容为 NORMAL，未知值拒绝。

当前只准备类型和命令边界。FM 的来源数据、自动续播、反馈和模式/清空命令限制随 HOME_PLAN 接入；entryId 不作为推荐 ID，应用尚无启动 FM 的用户入口。
