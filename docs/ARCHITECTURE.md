...
会话 extras 只包含当前项、数量、播放状态及队列内容版本；QueueController 仅在队列面板可见时通过服务命令读取 QueuePages 的有界 JSON 分页，版本变化丢弃旧批次，完整读取后才更新列表。进度轮询不重新解码整表。临时文件和恢复快照均按实际编码字节检查 4 MiB 上限，分页为 24 KiB；超过限制明确失败，不静默截断。普通/FM 状态边界尚未实现，详见 QUEUE_PREPARATION_PLAN.md。
