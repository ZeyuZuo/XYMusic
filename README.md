...
**用户复测反馈登录已恢复正常；在线搜索、实际播放、歌词、推荐和歌单尚未接通。** 同手机号多账号暂支持手动指定账号 ID，自动账号列表及头像未接入。首页两张推荐卡片显示真实未接入状态，不展示假数据。
...
登录后界面已调整为圆形账号图标、昵称与状态标签；账号操作收进菜单，开发服务配置默认折叠。浅色、深色与 1.5 倍字体 Preview 已检查，示例数据仅用于预览。
