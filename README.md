...
播放源解析、服务生命周期和界面控制分离；构建、41 项单元测试及 lint 通过。详细边界与手动验收清单见 [播放计划](docs/PLAYBACK_PLAN.md)。完整播放页已在阶段 2 实现；队列与切歌已在阶段 3 实现；进程恢复和歌词待后续阶段。
...
播放按 [播放功能实施计划](docs/PLAYBACK_PLAN.md) 分阶段推进：点歌出声 → 完整播放页 → 队列 → 恢复与稳定性 → LRC。阶段 1–3 已实现，完成手动验收后进入阶段 4。

先按播放计划验收基础点播，再按 [v0.1 产品与实施计划](docs/V0.1.md) 补齐恢复、稳定性与后续能力；登录恢复和续期的剩余验收见 [登录验收说明](docs/LOGIN.md)。另见 [架构和里程碑](docs/ARCHITECTURE.md)、[第三方复用记录](docs/THIRD_PARTY.md) 和 [开发约定](AGENTS.md)。
