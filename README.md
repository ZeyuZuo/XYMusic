...
**用户复测反馈登录已恢复正常；已实现歌曲、歌单、歌手三类在线搜索，实际播放、歌词、推荐和云端音乐库尚未接通。** 同手机号多账号暂支持手动指定账号 ID，自动账号列表未接入。首页两张推荐卡片显示真实未接入状态，不展示假数据。
...
`feature/search-playback` 分支先实现搜索：输入关键词后点击按钮或键盘搜索，切换歌曲、歌单、歌手标签查看对应结果，支持清空、手动加载更多、空结果与失败重试。列表每条独立排列，标题、完整歌手列表和专辑信息允许换行，不设置固定行高或单行截断。结果暂不提供播放、歌单详情或歌手主页跳转；搜索历史后续补齐。
...
先手动验收三类搜索，再按 [v0.1 产品与实施计划](docs/V0.1.md) 接通播放地址解析与 MediaController；登录恢复和续期的剩余验收见 [登录验收说明](docs/LOGIN.md)。另见 [架构和里程碑](docs/ARCHITECTURE.md)、[第三方复用记录](docs/THIRD_PARTY.md) 和 [开发约定](AGENTS.md)。
