...
**用户复测反馈登录已恢复正常；已实现歌曲、歌单、歌手三类在线搜索，已接通基础单曲播放与迷你播放器，歌词、推荐和云端音乐库尚未接通。** 同手机号多账号暂支持手动指定账号 ID，自动账号列表未接入。首页两张推荐卡片显示真实未接入状态，不展示假数据。
...
播放源解析、服务生命周期和界面控制分离；构建、41 项单元测试及 lint 通过。详细边界与手动验收清单见 [播放计划](docs/PLAYBACK_PLAN.md)。完整播放页、队列、进程恢复和歌词待后续阶段。
...
播放按 [播放功能实施计划](docs/PLAYBACK_PLAN.md) 分阶段推进：点歌出声 → 完整播放页 → 队列 → 恢复与稳定性 → LRC。阶段 1 已实现，完成手动验收后进入阶段 2。

先按播放计划验收基础点播，再按 [v0.1 产品与实施计划](docs/V0.1.md) 补齐完整播放页与后续能力；登录恢复和续期的剩余验收见 [登录验收说明](docs/LOGIN.md)。另见 [架构和里程碑](docs/ARCHITECTURE.md)、[第三方复用记录](docs/THIRD_PARTY.md) 和 [开发约定](AGENTS.md)。
